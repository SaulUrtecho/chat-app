...
import 'package:firebase_chat/data/remote/models/group_model.dart';
import 'package:firebase_chat/data/remote/models/my_chat_model.dart';
import 'package:firebase_chat/data/remote/models/text_message_model.dart';
import 'package:firebase_chat/data/remote/models/user_model.dart';
...
class FirebaseRemoteDataSourceImpl implements FirebaseRemoteDataSource {
  ...
  Future<void> verifyPhoneNumber(String phoneNumber) async {
    ...
    final PhoneVerificationFailed phoneVerificationFailed = (FirebaseAuthException authCredential) {
  ...
  Future<String> getChannelId(EngageUserEntity engageUserEntity) {
    ...
    print("uid ${engageUserEntity.uid} - otherUid ${engageUserEntity.otherUid}");
  ...
  Future<String> createOneToOneChatChannel(EngageUserEntity engageUserEntity) async {
    ...
    userCollectionRef
      ...
      //currentUser
      userCollectionRef.doc(engageUserEntity.uid).collection('chatChannel').doc(engageUserEntity.otherUid).set(channel);

      //otherUser
      userCollectionRef.doc(engageUserEntity.otherUid).collection('chatChannel').doc(engageUserEntity.uid).set(channel);
  ...
  Future<void> sendTextMessage(TextMessageEntity textMessageEntity, String channelId) async {
    final messagesRef = fireStore.collection("groupChatChannel").doc(channelId).collection("messages");
  ...
  Stream<List<TextMessageEntity>> getMessages(String channelId) {
    ...
    final messagesRef = oneToOneChatChannelRef.doc(channelId).collection("messages");

    return messagesRef
        .orderBy('time')
        .snapshots()
        .map((querySnap) => querySnap.docs.map((queryDoc) => TextMessageModel.fromSnapshot(queryDoc)).toList());
  }

  @override
  Future<void> addToMyChat(MyChatEntity myChatEntity) async {
    final myChatRef = fireStore.collection("users").doc(myChatEntity.senderUID).collection("myChat");
    final otherChatRef = fireStore.collection("users").doc(myChatEntity.recipientUID).collection("myChat");
  ...
  Stream<List<MyChatEntity>> getMyChat(String uid) {
    final myChatRef = fireStore.collection("users").doc(uid).collection("myChat");
  ...
  @override
  Future<void> createNewGroup(MyChatEntity myChatEntity, List<String> selectUserList) async {
  ...
  Future<void> getUpdateUser(UserEntity user) async {
    ...
    if (user.profileUrl != null && user.profileUrl != "") userInformation['profileUrl'] = user.profileUrl;
    if (user.status != null && user.status != "") userInformation['status'] = user.status;
    if (user.phoneNumber != null && user.phoneNumber != "") userInformation["phoneNumber"] = user.phoneNumber;
    if (user.name != null && user.name != "") userInformation["name"] = user.name;
  ...
  Stream<List<GroupEntity>> getGroups() {
    ...
    return groupCollection
        ...
        .snapshots()
        .map((querySnapshot) => querySnapshot.docs.map((e) => GroupModel.fromSnapshot(e)).toList());
  }
  ...
  Future<void> joinGroup(GroupEntity groupEntity) async {
    ...
    groupChatChannelCollection.doc(groupEntity.groupId).get().then((groupChannel) {
  ...
  Future<void> updateGroup(GroupEntity groupEntity) async {
    ...
    if (groupEntity.groupProfileImage != null && groupEntity.groupProfileImage != "")
    ...
    if (groupEntity.creationTime != null) groupInformation["creationTime"] = groupEntity.creationTime;
